...
// The LikeVideoFunc type is an adapter to allow the use of ordinary
// function as LikeVideo mutator.
type LikeVideoFunc func(context.Context, *ent.LikeVideoMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f LikeVideoFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.LikeVideoMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.LikeVideoMutation", m)
}
