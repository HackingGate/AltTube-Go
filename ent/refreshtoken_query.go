...
func (rtq *RefreshTokenQuery) QueryAccessTokens() *AccessTokenQuery {
	query := (&AccessTokenClient{config: rtq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := rtq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (rtq *RefreshTokenQuery) Clone() *RefreshTokenQuery {
	...
	return &RefreshTokenQuery{
		...
		withAccessTokens: rtq.withAccessTokens.Clone(),
...
func (rtq *RefreshTokenQuery) WithAccessTokens(opts ...func(*AccessTokenQuery)) *RefreshTokenQuery {
	query := (&AccessTokenClient{config: rtq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	rtq.withAccessTokens = query
	return rtq
}
...
func (rtq *RefreshTokenQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*RefreshToken, error) {
	...
	if query := rtq.withAccessTokens; query != nil {
		if err := rtq.loadAccessTokens(ctx, query, nodes,
			func(n *RefreshToken) { n.Edges.AccessTokens = []*AccessToken{} },
			func(n *RefreshToken, e *AccessToken) { n.Edges.AccessTokens = append(n.Edges.AccessTokens, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
