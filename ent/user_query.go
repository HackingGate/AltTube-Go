...
func (uq *UserQuery) QueryLikeVideos() *LikeVideoQuery {
	query := (&LikeVideoClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withLikeVideos:    uq.withLikeVideos.Clone(),
...
func (uq *UserQuery) WithLikeVideos(opts ...func(*LikeVideoQuery)) *UserQuery {
	query := (&LikeVideoClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	uq.withLikeVideos = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withLikeVideos; query != nil {
		if err := uq.loadLikeVideos(ctx, query, nodes,
			func(n *User) { n.Edges.LikeVideos = []*LikeVideo{} },
			func(n *User, e *LikeVideo) { n.Edges.LikeVideos = append(n.Edges.LikeVideos, e) }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadLikeVideos(ctx context.Context, query *LikeVideoQuery, nodes []*User, init func(*User), assign func(*User, *LikeVideo)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(likevideo.FieldUserID)
	}
	query.Where(predicate.LikeVideo(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.LikeVideosColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
